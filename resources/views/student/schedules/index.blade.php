...
<head>
    ...
    <title>Jadwal - Portal Akademik</title>
...
<body>
    ...
    <div class="main-content">
        ...
        <div class="d-flex justify-content-between align-items-center mb-4">
            <div class="d-flex align-items-center gap-3">
                ...
                <div>
                    ...
                    <small class="text-muted">Jadwal perkuliahan semester ini</small>
                </div>
            </div>
        </div>
        ...
        @forelse($dayOrder as $day)
            ...
            <div class="mb-4">
                <h6 class="fw-bold text-muted mb-3">
                    <i class="bi bi-calendar-day me-2"></i>{{ $days[$day] }}
                </h6>
                <div class="row g-3">
                    @foreach($grouped[$day] as $schedule)
                    <div class="col-md-4">
                        <div class="card p-3 h-100 shadow-sm" style="border-left: 4px solid #1e40af;">
                            <h6 class="fw-bold mb-1 text-wrap">{{ $schedule->course->name }}</h6>
                            <div class="d-flex flex-column gap-1 mt-2">
                                <small class="text-muted text-break"><i class="bi bi-tag me-1"></i>{{ $schedule->course->code }}</small>
                                <small class="text-muted"><i class="bi bi-clock me-1"></i>{{ \Carbon\Carbon::parse($schedule->start_time)->format('H:i') }} - {{ \Carbon\Carbon::parse($schedule->end_time)->format('H:i') }}</small>
                                <small class="text-muted text-break"><i class="bi bi-geo-alt me-1"></i>{{ $schedule->room ?? '-' }}</small>
                                <small class="text-muted text-break"><i class="bi bi-person me-1"></i>{{ $schedule->lecturer->name }}</small>
                                <small class="text-muted"><i class="bi bi-layers me-1"></i>{{ $schedule->course->credits }} SKS</small>
                            </div>
                        </div>
                    </div>
                    @endforeach
        ...
        <div class="card p-5 text-center text-muted">
            ...
            <p class="mb-0">Belum ada jadwal kuliah</p>
        </div>
        @endforelse
    </div>

...
<body>
    ...
    <!-- Perbaikan: Tag penutup sidebar berhasil ditambahkan -->
                        ...
                        <tbody>
                            ...
                            @forelse($schedules as $index => $schedule)
                                <tr>
                                    ...
                                    <td>
                                        <span
                                            class="fw-semibold text-dark">{{ $schedule->course?->name ?? '-' }}</span>
                                        @if ($schedule->course?->code)
                                            <small class="text-muted d-block"
                                                style="font-size: 0.75rem;">{{ $schedule->course->code }}</small>
                                        @endif
                                    </td>
                                    <td class="text-secondary"><i class="bi bi-person me-1"></i>
                                        {{ $schedule->lecturer?->name ?? '-' }}</td>
                                    <td><span
                                            class="badge bg-secondary bg-opacity-10 text-secondary px-2 py-1">{{ $days[$schedule->day] ?? $schedule->day }}</span>
                                    </td>
                                    ...
                                    <td><i class="bi bi-geo-alt me-1 text-muted"></i> {{ $schedule->room ?? '-' }}
                                    </td>
                                    <td><span class="badge bg-primary bg-opacity-10 text-primary">Smtr
                                            {{ $schedule->semester }}</span></td>
                                    <td>
                                        @if ($schedule->status == 'active')
                                            <span
                                                class="badge bg-success bg-opacity-10 text-success px-2.5 py-1.5 rounded">
                                                <i class="bi bi-check2-circle me-1"></i> Aktif
                                            </span>
                                        @else
                                            <span
                                                class="badge bg-secondary bg-opacity-10 text-secondary px-2.5 py-1.5 rounded">
                                    ...
                                    <td>
                                        <div class="d-flex gap-2 justify-content-center pe-3">
                                            <a href="{{ route('lecturer.schedules.edit', $schedule->id) }}"
                                                class="btn btn-action btn-outline-primary btn-sm" title="Edit">
                                            ...
                                            <form action="{{ route('lecturer.schedules.destroy', $schedule->id) }}"
                                                ...
                                                @method('DELETE')
                                                <button type="submit"
                                                    class="btn btn-action btn-outline-danger btn-sm" title="Hapus">
